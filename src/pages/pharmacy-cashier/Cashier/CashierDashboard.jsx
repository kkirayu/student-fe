...
import { Wallet, CreditCard, Clock, CheckCircle } from 'lucide-react';

const CashierDashboard = () => {
  // 1. Statistik Ringkasan Shift Aktif
  const shiftStats = [
    {
      title: 'Total Uang di Laci (Tunai)',
      value: 'Rp 1.850.000',
      detail: 'Kas Awal: Rp 500.000',
      icon: <Wallet className="text-blue-600 h-6 w-6" />,
      bgIcon: 'bg-blue-100',
    },
    {
      title: 'Pemasukan Non-Tunai',
      value: 'Rp 1.200.000',
      detail: '6 Transaksi Terproses',
      icon: <CreditCard className="text-emerald-600 h-6 w-6" />,
      bgIcon: 'bg-emerald-100',
    },
    {
      title: 'Antrean Menunggu',
      value: '4 Pasien',
      detail: 'Segera panggil',
      icon: <Clock className="text-orange-600 h-6 w-6" />,
      bgIcon: 'bg-orange-100',
    },
    {
      title: 'Selesai Dibayar (Shift Ini)',
      value: '14 Pasien',
      detail: 'Sesi: Pagi (07:00 - 14:00)',
      icon: <CheckCircle className="text-purple-600 h-6 w-6" />,
      bgIcon: 'bg-purple-100',
    },
  ];

  // 2. Data Mockup Antrean Kasir Hari Ini
  const queueList = [
    { id: 'Q-012', name: 'Boni (Anjing)', owner: 'Budi', total: 'Rp 350.000', status: 'Menunggu' },
    { id: 'Q-013', name: 'Miko (Kucing)', owner: 'Siti', total: 'Rp 120.000', status: 'Menunggu' },
    { id: 'Q-014', name: 'Cleo (Kucing)', owner: 'Andi', total: 'Rp 540.000', status: 'Menunggu' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Petugas Aktif: <span className="font-semibold text-slate-700">Siti Rahma (Shift 1)</span></p>
        </div>
        <div className="text-sm font-medium text-slate-500 bg-slate-100 px-3 py-1.5 rounded-sm self-start sm:self-center">
          Live Update: 20 Mei 2026 - 16:04
        </div>
      </div>

      {/* 2. Stat Cards Grid (Ringkasan Keuangan & Antrean Shift) */}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 text-slate-400 font-medium">
                  <th className="pb-3">No. Antrean</th>
                  <th className="pb-3">Nama Hewan (Pemilik)</th>
                  <th className="pb-3">Estimasi Tagihan</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {queueList.map((queue, i) => (
                  <tr key={i} className="hover:bg-slate-50/70">
                    <td className="py-3.5 font-bold text-blue-600">{queue.id}</td>
                    <td className="py-3.5">
                      <div className="font-semibold text-slate-800">{queue.name}</div>
                      <div className="text-xs text-slate-400">Pmlk: {queue.owner}</div>
                    </td>
                    <td className="py-3.5 font-medium text-slate-700">{queue.total}</td>
                    <td className="py-3.5 text-right">
                      <button className="rounded bg-blue-600 px-3 py-1.5 text-xs font-medium text-white shadow-sm hover:bg-blue-700 transition-colors">
                        Proses Bayar
                      </button>
                    </td>
                  </tr>
        ...
        {/* Kolom Kanan: Proporsi Metode Pembayaran Shift Ini */}
        <div className="col-span-1 rounded-sm border border-slate-200 bg-white p-6 shadow-sm lg:col-span-4">
          <h3 className="mb-1 text-xl font-bold text-black">Metode Pembayaran</h3>
          <p className="mb-4 text-xs text-slate-400">Volume transaksi berdasarkan jenis pembayaran shift ini</p>
          
          <div className="space-y-5">
            {[
              { label: 'Tunai (Cash)', value: 55, total: 'Rp 1.350.000', color: 'bg-blue-600' },
              { label: 'QRIS / E-Wallet', value: 30, total: 'Rp 900.000', color: 'bg-emerald-500' },
              { label: 'Debit / Kartu Kredit', value: 15, total: 'Rp 300.000', color: 'bg-orange-400' },
            ].map((item, i) => (
              <div key={i}>
                ...
                <div className="h-2.5 w-full rounded-full bg-slate-100 mb-1">
                  <div className={`h-2.5 rounded-full ${item.color}`} style={{ width: `${item.value}%` }}></div>
                </div>
                <div className="text-right text-xs text-slate-400">{item.total}</div>
              </div>
            ))}
          </div>

          {/* Tombol tambahan kasir */}
          <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col gap-2">
            <button className="w-full text-center rounded border border-slate-200 bg-white py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
              Input Kas Masuk / Keluar
            </button>
            <button className="w-full text-center rounded bg-red-50 py-2 text-sm font-semibold text-red-600 hover:bg-red-100 transition-colors">
              Tutup Shift & Cetak Reclon
            </button>
          </div>
        </div>
